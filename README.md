...
#Sadlier for the HubSpot CMS by New Breed
...
```
hs upload src Sadlier
```

#### Watch entire project

```
hs watch src Sadlier
```

#### Fetch entire project with overwriting

```
hs fetch Sadlier src -o
```

#### Fetch a specific module add -o to the end to overwrite files

```
hs fetch Sadlier/modules/u4m-000.module src/modules/u4m-000.module -o
```

hs fetch Sadlier/modules/u4m-cards-side.module src/modules/u4m-cards-side.module -o

#### Watch entire folder on non-default portal (portal names are set in hubspot.config.yml)

```
hs watch --account=hug_vt src Sadlier
```
